...
def define_df_plot_conc(mix_omf_conc, mac_names, fig_title):
    # Create box plot fig with organic aerosol concentration for all species and stations
    conc_rename_list = []
    for conc in mix_omf_conc:
        conc_rename = utils_plots.rename_func(conc,
                                              '',
                                              'ECHAM-HAM aerosol concentration',
                                              'ECHAM-HAM model')  # reformat column names
        conc_rename = utils_plots.rename_func(conc_rename,
                                              '',
                                              'Observation aerosol concentration',
                                              'Observations')  # reformat column names
        conc_rename_list.append(conc_rename)
    fig, ax = plt.subplots(figsize=(13, 8))  # 15, 8
    box_plot_vert_only_conc(ax,
                           pd.concat([conc_rename_list[0],
                                      conc_rename_list[1],
                                      conc_rename_list[2]]),
                           mac_names,
                           [1e-7, 1e1])
...
def box_plot_vert_only_conc(ax, dict_df, mol_name, lim):
    """
    Plotting model values and ovservations of OMF and aerosol mass concentration
    :param ax: matplotlib axes
    :param dict_df: dataframe with observations and interpolated model values for all species and stations
    ...
    dict_df_conc = dict_df.copy()
    print(dict_df_conc[""])

    dict_df_conc = dict_df_conc[(dict_df_conc[""]=="ECHAM-HAM model")|
                                (dict_df_conc[""]=="Observations")]

    flier = utils_plots.get_marker_flier()
    bx = sns.boxplot(data=dict_df_conc,
                     x="Measurements",
                     y="Aerosol OMF",
                     hue="",
                     palette=my_pal,
    ...
    ax.grid(linestyle='--', linewidth=0.4)
    ax.set_ylim(lim)
    ax.set_ylabel('Concentration (µg m$^{-3}$)', fontsize=f)
    # color axis
